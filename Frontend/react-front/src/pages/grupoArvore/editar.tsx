...
const EditarGrupoArvore = ({location}:any) => {
    ...
    useEffect(() => {
        setNome(location.state.grupoArvore.nome);
        setDescricao(location.state.grupoArvore.descricao);
        setArvoresSelecionadas(location.state.grupoArvore.arvores.map((item:any) => (item.codigo)));
    }, []);
    ...
    async function handleSubmit(event: FormEvent){
        ...
        const codigo = location.state.grupoArvore.codigo;
        const data = {
            nome,
            descricao,
            arvores,
                    ...
                    {arvoresSelecionadas.map((arvore, idx) => (
                        <div  id={`divTotal${idx}`}>
                            ...
                            <select id={`arvores${idx}`} value={arvore} onChange={(val) => handleSelectArvore(idx, val)}>
                                ...
                                {arvores.map(arv => (
                                    <option value={arv.codigo}>{arv.descricao}</option>
                                ))}
                            </select>
...
export default EditarGrupoArvore;